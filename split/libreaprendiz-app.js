    ...
    function createEmptyFacilitadorSuplenciaState() {
      return {
        suplencia_id: '',
        facilitador_titular_id: '',
        facilitador_suplente_id: '',
        grupo_id: '',
        materia_id: '',
        taller_id: '',
        uiMode: 'grupo', // UI only: 'grupo' | 'taller'
        fecha_inicio: '',
        fecha_fin: ''
      };
    }
    ...
    function createEmptyEval360UiState() {
      return {
        admin: {
          loaded: false,
          loading: false,
    ...
    const ADMIN_PLANEACIONES_IDLE_WARMUP_LIMIT = PLANEACIONES_PAGE_SIZE;
    ...
    const state = {
      ...
      eval360Ui: createEmptyEval360UiState(),
      planEditor: {
        mode: 'create',
        planId: '',
        lockedSemanaId: '',
    ...
    function getCatalogBlocksForModuleWithScope(moduleName, options = {}) {
      ...
      switch (String(moduleName || '').trim()) {
        ...
        case 'facilitadores':
          return ['facilitadores', 'facilitadores_admin', 'facilitador_asignaciones', 'grupos', 'materias', 'semanas', 'talleres_admin', 'suplencias'];
        case 'materias':
    ...
    function hasAdminPlaneacionesWarmupWork() {
      if (!canUseAdminShell() || !state.ui) return false;
      if (state.ui.planeacionesLoaded) return false;
      if (state.ui.adminPlaneacionesWarmupDone) return false;
      return true;
    }
    ...
    async function warmAdminPlaneacionesList() {
      if (!canUseAdminShell() || !state.ui) return;
      const data = await scheduleAfterPaint(() => api('getPlaneaciones', {
        limit: ADMIN_PLANEACIONES_IDLE_WARMUP_LIMIT,
        offset: 0,
        include_detail: false
      }), 120);
      if (!canUseAdminShell() || !state.ui) return;
      const activeModule = String(state.activeAdminModule || '').trim();
      const canApplyWarmup = activeModule === 'dashboard' ||
        (activeModule === 'planeaciones' && !hasActivePlaneacionesFilters());
      if (!canApplyWarmup) return;
      const rows = Array.isArray(data && data.rows) ? data.rows : [];
      state.planeaciones = preserveOpenPlanDetailOnRowsReplace(rows);
      state.ui.planeacionesLoaded = true;
      state.ui.planeacionesOffset = rows.length;
      state.ui.planeacionesHasMore = !!(data && data.has_more);
      state.ui.planeacionesLoading = false;
      state.ui.planeacionesLoadingMore = false;
      state.ui.adminPlaneacionesWarmupDone = true;
      persistCurrentBootSnapshot('planeaciones');
      if (activeModule === 'planeaciones') {
        renderPlaneacionesSurface({ includeStats: true, includePlaneaciones: true, includeAlertas: false });
    ...
    function getAdminModulePlaceholderCopy(moduleName) {
      switch (String(moduleName || '').trim()) {
        ...
        case 'eval360':
          return {
            title: 'Cargando Evaluaci\u00f3n 360',
            body: 'Consultamos este m\u00f3dulo solo bajo demanda para no afectar planeaciones.'
          };
        default:
          return {
    ...
    function activateAdminModule(moduleName) {
      ...
      renderAdminShell();
      if (nextModule === 'eval360') {
        renderAdminEval360Module();
        const evalAdmin = getEval360Ui().admin;
        if (!evalAdmin.loaded && !evalAdmin.loading) {
          loadEval360AdminModule().catch((err) => {
            evalAdmin.loading = false;
            evalAdmin.loaded = true;
            evalAdmin.error = formatApiError(err);
            renderAdminEval360Module();
          });
        }
        syncRoleUi();
      ...
      if (nextModule === 'planeaciones' && state.ui && !state.ui.planeacionesLoaded) {
        const pendingPlaneacionesWarmup = state.ui.adminPlaneacionesWarmupPromise;
        if (pendingPlaneacionesWarmup) {
          state.ui.planeacionesLoading = true;
          renderPlaneacionesList();
        }
        const loadPlaneaciones = pendingPlaneacionesWarmup
          ? pendingPlaneacionesWarmup.then(() => {
              if (state.ui && state.ui.planeacionesLoaded) return null;
              return refreshPlaneaciones();
            })
          : refreshPlaneaciones();
        loadPlaneaciones
    ...
    function renderAdminShell() {
      ...
      if ($('adminCountEval360')) {
        const evalAdmin = getEval360Ui().admin;
        $('adminCountEval360').textContent = evalAdmin.loaded && evalAdmin.feature && evalAdmin.feature.enabled === true
          ? String((evalAdmin.invitaciones || []).length || 'On')
          : 'Off';
      }
    ...
    // ─── Suplencias helpers ──────────────────────────────
    function getFacilitadorSuplencias(facilitadorId, options = {}) {
      const includeArchived = !!options.includeArchived;
      return (Array.isArray(state.catalogos.suplencias) ? state.catalogos.suplencias : [])
        .filter((row) => String(row.facilitador_titular_id || '').trim() === String(facilitadorId || '').trim())
    ...
    function getSuplenciaLabel(suplencia) {
      ...
      if (tallerId) {
        const taller = ((state.catalogos.talleres_admin || state.catalogos.talleres || []).find((t) => t.taller_id === tallerId) || {});
        return 'Taller: ' + (taller.nombre || tallerId);
      }
      const materia = ((state.catalogos.materias || state.catalogos.materias_admin || []).find((m) => m.materia_id === suplencia.materia_id) || {});
      return getGrupoNombre(suplencia.grupo_id) + ' · ' + (materia.nombre || suplencia.materia_id || '');
    }

    function applySavedSuplenciaCatalogRow(row) {
      if (!row || !row.suplencia_id) return;
      const normalized = {
        suplencia_id: String(row.suplencia_id || '').trim(),
        facilitador_titular_id: String(row.facilitador_titular_id || '').trim(),
        facilitador_suplente_id: String(row.facilitador_suplente_id || '').trim(),
        grupo_id: String(row.grupo_id || '').trim(),
        materia_id: String(row.materia_id || '').trim(),
        taller_id: String(row.taller_id || '').trim(),
        fecha_inicio: toYmdFrontend_(row.fecha_inicio || ''),
        fecha_fin: toYmdFrontend_(row.fecha_fin || ''),
        activa: typeof row.activa === 'boolean' ? row.activa : isTruthyValue(row.activa),
        fecha_creacion: row.fecha_creacion || '',
        fecha_actualizacion: row.fecha_actualizacion || '',
        creada_por: String(row.creada_por || '').trim(),
        actualizada_por: String(row.actualizada_por || '').trim(),
    ...
    function renderFacilitadorDetailPanel() {
      ...
      host.innerHTML = [
        '<div class="admin-facilitadores-summary">',
          ...
          '<div class="admin-alumnos-section-head"><div><h4>Suplencias</h4><div class="subtle">Cuando otro facilitador cubre temporalmente las planeaciones de este titular.</div></div>' +
            '<div class="actions compact">' + (canUseAdminShell() ? '<button class="btn-primary" type="button" onclick="openFacilitadorSuplenciaEditor(\'new\')">Nueva suplencia</button>' : '') + '</div></div>',
          '<div id="adminFacilitadorSuplenciasList">' + renderFacilitadorSuplenciasList(facilitador.facilitador_id) + '</div>',
          (state.facilitadoresUi.suplenciaOpen ? renderFacilitadorSuplenciaEditor() : ''),
    ...
    function renderFacilitadorSuplenciasList(facilitadorId) {
      const rows = getFacilitadorSuplencias(facilitadorId);
      if (!rows.length) {
        return '<div class="admin-alumnos-empty" style="min-height:80px;"><div><strong>Sin suplencias configuradas.</strong></div></div>';
      }
      return rows.map((row) => {
        const suplente = getFacilitadorById(row.facilitador_suplente_id);
        const suplenteName = suplente
          ? (suplente.nombre_mostrado || suplente.nombre_completo || row.facilitador_suplente_id)
          : row.facilitador_suplente_id;
        const assignLabel = getSuplenciaLabel(row);
        const periodo = [
          row.fecha_inicio ? formatFechaCorta(row.fecha_inicio) : '',
          row.fecha_fin ? formatFechaCorta(row.fecha_fin) : ''
        ].filter(Boolean).join(' - ') || 'Sin vigencia';
        const archivedBadge = row.archivada_at ? ' <span class="admin-alumnos-badge is-archived">Archivada</span>' : '';
        const inactiveBadge = !row.activa && !row.archivada_at ? ' <span class="admin-alumnos-badge is-inactive">Inactiva</span>' : '';
        return [
          '<article class="admin-facilitadores-assignment-item">',
            '<div class="admin-facilitadores-assignment-copy">',
              '<strong>' + escapeHtml(suplenteName) + archivedBadge + inactiveBadge + '</strong>',
              '<div class="mini">' + escapeHtml(assignLabel) + ' &middot; ' + escapeHtml(periodo) + '</div>',
            '</div>',
            '<div class="admin-facilitadores-assignment-actions">',
              (!row.archivada_at ? '<button class="btn-ghost" type="button" onclick="openFacilitadorSuplenciaEditor(\'edit\', \'' + escapeJsAttrValue(row.suplencia_id) + '\')">Editar</button>' : ''),
              (!row.archivada_at ? '<button class="btn-accent" type="button" onclick="archiveFacilitadorSuplencia(this, \'' + escapeJsAttrValue(row.suplencia_id) + '\')">Quitar</button>' : ''),
            '</div>',
          '</article>'
        ].join('');
      }).join('');
    ...
    function renderFacilitadorSuplenciaEditor() {
      const sup = state.facilitadoresUi.suplencia || createEmptyFacilitadorSuplenciaState();
      const esTaller = !!(sup.taller_id || sup.uiMode === 'taller');
      const titularId = state.facilitadoresUi.selectedFacilitadorId || sup.facilitador_titular_id;
      return [
        '<section id="adminFacilitadorSuplenciaEditorPanel" class="admin-alumnos-panel">',
          '<div class="admin-alumnos-panel-head"><div><h4>' + escapeHtml(sup.suplencia_id ? 'Editar suplencia' : 'Nueva suplencia') + '</h4><div class="subtle">Define quién cubre y qué asignación, con vigencia de fechas.</div></div></div>',
          '<div class="admin-alumnos-mini-grid">',
            '<label class="field admin-alumnos-field-full">',
              '<span>Facilitador suplente</span>',
              '<select id="adminFacilitadorSuplenciaSuplenteSelect">',
                renderSuplenteFacilitadorOptions(titularId, sup.facilitador_suplente_id),
              '</select>',
            '</label>',
            '<label class="field admin-alumnos-field-full">',
              '<span>Tipo de asignación</span>',
              '<select id="adminFacilitadorSuplenciaTipo" onchange="onFacilitadorSuplenciaTipoChange(this.value)">',
                '<option value="grupo"' + (!esTaller ? ' selected' : '') + '>Grupo + Materia</option>',
                '<option value="taller"' + (esTaller ? ' selected' : '') + '>Taller</option>',
              '</select>',
            '</label>',
            '<div id="adminFacilitadorSuplenciaGrupoBlock"' + (esTaller ? ' style="display:none"' : '') + '>',
              '<label class="field">',
                '<span>Grupo</span>',
                '<select id="adminFacilitadorSuplenciaGrupo"></select>',
              '</label>',
            '</div>',
            '<div id="adminFacilitadorSuplenciaMateriaBlock"' + (esTaller ? ' style="display:none"' : '') + '>',
              '<label class="field">',
                '<span>Materia</span>',
                '<select id="adminFacilitadorSuplenciaMateria"></select>',
              '</label>',
            '</div>',
            '<div id="adminFacilitadorSuplenciaTallerBlock"' + (!esTaller ? ' style="display:none"' : '') + '>',
              '<label class="field">',
                '<span>Taller</span>',
                '<select id="adminFacilitadorSuplenciaTaller"></select>',
              '</label>',
            '</div>',
            '<label class="field">',
              '<span>Fecha de inicio</span>',
              '<input id="adminFacilitadorSuplenciaInicio" type="date" value="' + escapeHtml(sup.fecha_inicio || '') + '">',
            '</label>',
            '<label class="field">',
              '<span>Fecha de fin</span>',
              '<input id="adminFacilitadorSuplenciaFin" type="date" value="' + escapeHtml(sup.fecha_fin || '') + '">',
            '</label>',
          '</div>',
          '<div class="actions compact admin-alumnos-panel-actions">',
            '<button class="btn-ghost" type="button" onclick="closeFacilitadorSuplenciaPanel()">Cancelar</button>',
            '<button class="btn-primary" type="button" onclick="saveFacilitadorSuplencia(this)">Guardar suplencia</button>',
          '</div>',
        '</section>'
      ].join('');
    }

    function renderSuplenteFacilitadorOptions(titularId, selectedId) {
      const facs = (state.catalogos.facilitadores_admin || state.catalogos.facilitadores || [])
        .filter((f) => {
          if (!isTruthyValue(f.activo)) return false;
          if (String(f.facilitador_id || '').trim() === String(titularId || '').trim()) return false;
          return true;
        });
      return facs.map((f) => {
        const id = String(f.facilitador_id || '').trim();
        const nombre = f.nombre_mostrado || f.nombre_completo || id;
        const sel = id === String(selectedId || '').trim() ? ' selected' : '';
        return '<option value="' + escapeHtml(id) + '"' + sel + '>' + escapeHtml(nombre) + '</option>';
      }).join('');
    }

    function onFacilitadorSuplenciaTipoChange(tipo) {
      const grupoBlock = $('adminFacilitadorSuplenciaGrupoBlock');
      const materiaBlock = $('adminFacilitadorSuplenciaMateriaBlock');
      const tallerBlock = $('adminFacilitadorSuplenciaTallerBlock');
    ...
    function openFacilitadorSuplenciaEditor(mode, suplenciaId) {
      const titularId = state.facilitadoresUi.selectedFacilitadorId;
      let sup = createEmptyFacilitadorSuplenciaState();
      sup.facilitador_titular_id = titularId;
      if (mode === 'edit' && suplenciaId) {
        const existing = (Array.isArray(state.catalogos.suplencias) ? state.catalogos.suplencias : [])
          .find((r) => r.suplencia_id === suplenciaId);
        if (existing) {
          sup = Object.assign({}, sup, {
            suplencia_id: existing.suplencia_id,
            facilitador_titular_id: existing.facilitador_titular_id || titularId,
            facilitador_suplente_id: existing.facilitador_suplente_id,
            grupo_id: existing.grupo_id,
            materia_id: existing.materia_id,
            taller_id: existing.taller_id,
            uiMode: existing.taller_id ? 'taller' : 'grupo',
      ...
      if (grupoSel) {
        const grupos = (state.catalogos.grupos || []);
        grupoSel.innerHTML = grupos.map((g) => {
          const sel = g.grupo_id === sup.grupo_id ? ' selected' : '';
          return '<option value="' + escapeHtml(g.grupo_id) + '"' + sel + '>' + escapeHtml(g.nombre_grupo || g.grupo_id) + '</option>';
        }).join('');
      }
      const materiaSel = $('adminFacilitadorSuplenciaMateria');
      if (materiaSel) {
        const materias = (state.catalogos.materias_admin || state.catalogos.materias || []).filter((m) => m.estatus === 'activa' || m.activo);
        materiaSel.innerHTML = materias.map((m) => {
          const sel = m.materia_id === sup.materia_id ? ' selected' : '';
          return '<option value="' + escapeHtml(m.materia_id) + '"' + sel + '>' + escapeHtml(m.nombre || m.materia_id) + '</option>';
        }).join('');
      }
      const tallerSel = $('adminFacilitadorSuplenciaTaller');
      if (tallerSel) {
        const talleres = (state.catalogos.talleres_admin || state.catalogos.talleres || [])
          .filter((t) => t.estatus === 'activo' || t.activo);
        tallerSel.innerHTML = talleres.map((t) => {
          const sel = t.taller_id === sup.taller_id ? ' selected' : '';
          return '<option value="' + escapeHtml(t.taller_id) + '"' + sel + '>' + escapeHtml(t.nombre || t.taller_id) + '</option>';
        }).join('');
      }
    }
    ...
    async function saveFacilitadorSuplencia(button) {
      const titularId = state.facilitadoresUi.selectedFacilitadorId;
      const sup = state.facilitadoresUi.suplencia || {};
      const esTaller = $('adminFacilitadorSuplenciaTipo') && $('adminFacilitadorSuplenciaTipo').value === 'taller';
      const payload = {
        suplencia_id: sup.suplencia_id || '',
        facilitador_titular_id: titularId,
        facilitador_suplente_id: ($('adminFacilitadorSuplenciaSuplenteSelect') || {}).value || '',
        grupo_id: !esTaller && $('adminFacilitadorSuplenciaGrupo') ? $('adminFacilitadorSuplenciaGrupo').value : '',
        materia_id: !esTaller && $('adminFacilitadorSuplenciaMateria') ? $('adminFacilitadorSuplenciaMateria').value : '',
        taller_id: esTaller && $('adminFacilitadorSuplenciaTaller') ? $('adminFacilitadorSuplenciaTaller').value : '',
        fecha_inicio: ($('adminFacilitadorSuplenciaInicio') || {}).value || '',
        fecha_fin: ($('adminFacilitadorSuplenciaFin') || {}).value || '',
        activa: true
      };
      await handleAction('guardarSuplencia', async () => {
        const data = await api('guardarSuplencia', payload);
        if (data && data.suplencia_id) {
          // Refresh catalog row
          const saved = Object.assign({}, payload, { suplencia_id: data.suplencia_id, activa: true });
          applySavedSuplenciaCatalogRow(saved);
        }
        state.facilitadoresUi.suplenciaOpen = false;
        state.facilitadoresUi.suplencia = null;
        renderFacilitadorDetailPanel();
    ...
    async function archiveFacilitadorSuplencia(button, suplenciaId) {
      if (!confirm('¿Archivar esta suplencia? El suplente ya no podrá cubrir planeaciones de este titular.')) return;
      await handleAction('archivarSuplencia_' + suplenciaId, async () => {
        await api('archivarSuplencia', { suplencia_id: suplenciaId });
        // Remove from catalog
        if (Array.isArray(state.catalogos.suplencias)) {
          const idx = state.catalogos.suplencias.findIndex((r) => r.suplencia_id === suplenciaId);
          if (idx !== -1) {
            state.catalogos.suplencias[idx] = Object.assign({}, state.catalogos.suplencias[idx], {
              archivada_at: new Date().toISOString()
            });
          }
        }
        const listEl = $('adminFacilitadorSuplenciasList');
        if (listEl) listEl.innerHTML = renderFacilitadorSuplenciasList(state.facilitadoresUi.selectedFacilitadorId);
        setBanner('Suplencia archivada.', 'success');
      }, { button, busyText: 'Archivando...' });
    }

    // ─── Facilitador pulse — Suplencias activas section ─────────────────────

    function renderFacilitadorSuplenciasActivasSection(facilitadorId) {
      const suplencias = (Array.isArray(state.catalogos.suplencias) ? state.catalogos.suplencias : [])
        .filter((s) => {
          return String(s.facilitador_suplente_id || '').trim() === String(facilitadorId || '').trim() &&
            s.activa && !String(s.archivada_at || '').trim();
        });
      if (!suplencias.length) return '';
      ...
      const rows = suplencias.map((sup) => {
        ...
        const cells = semanaHeaders.map((semana) => {
          ...
          const plan = (Array.isArray(state.planeaciones) ? state.planeaciones : []).find((p) => {
            ...
            if (sup.taller_id) return String(p.taller_id || '').trim() === sup.taller_id;
          ...
          if (plan) {
            ...
            const stateClass = estado === 'cerrada' || estado === 'archivada' ? 'is-closed'
              : estado === 'activa' ? 'is-ok'
              : estado === 'cierre_pendiente' ? 'is-pending'
              : 'is-missing';
            const stateLabel = estado === 'cerrada' ? 'Cerrada'
              : estado === 'activa' ? 'Lista'
    ...
    async function cubrirSuplencia(button, suplenciaId, semanaId) {
      await handleAction('crearPlaneacionComoSuplente_' + suplenciaId + '_' + semanaId, async () => {
        const requestId = uid ? uid('PLS') : ('PLS-' + Date.now());
        const data = await api('crearPlaneacionComoSuplente', {
            ...
            '<article id="plan-card-' + escapeHtml(plan.planeacion_id) + '" class="plan-card is-collapsed plan-card-compact">' +
              '<div class="plan-collapsed-mobile">' +
                '<div class="plan-top">' +
                  ...
                  '</div>' +
                  '<div class="plan-badges">' + badgeHtml + suplenciaBadgeHtml + '</div>' +
                '</div>' +
                '<div class="meta-grid">' +
    ...
    function getEval360Ui() {
      if (!state.eval360Ui) state.eval360Ui = createEmptyEval360UiState();
      return state.eval360Ui;
    }

    function normalizeEval360MomentFrontend(value) {
      return String(value || '').trim() === 'final' ? 'final' : 'inicio';
    }

    function getEval360PublicTokenFromUrl() {
      if (typeof window === 'undefined' || !window.location) return '';
      const params = new URLSearchParams(window.location.search || '');
      const queryToken = params.get('eval360_token') || params.get('token') || params.get('e360');
      if (queryToken) return String(queryToken).trim();
      const hash = String(window.location.hash || '').replace(/^#/, '');
      if (!hash) return '';
      if (hash.indexOf('eval360=') === 0) return decodeURIComponent(hash.slice('eval360='.length)).trim();
      if (hash.indexOf('eval360_token=') === 0) return decodeURIComponent(hash.slice('eval360_token='.length)).trim();
      return '';
    }

    function isEval360PublicRoute() {
      return !!getEval360PublicTokenFromUrl();
    }

    function formatEval360Score(value) {
      if (value === '' || value === null || value === undefined || Number.isNaN(Number(value))) return 'Sin dato';
      return String(Math.round(Number(value) * 100) / 100);
    }

    function eval360MeterWidth(value) {
      const numeric = Number(value);
      if (!Number.isFinite(numeric)) return 0;
      return Math.max(0, Math.min(100, (numeric / 4) * 100));
    }

    function formatEval360HumanLabel(value) {
      const clean = String(value || '').trim();
      if (!clean) return '';
      return clean.replace(/[_-]+/g, ' ').replace(/\b\w/g, (char) => char.toUpperCase());
    }

    function formatEval360AreaLabel(value) {
      const clean = String(value || '').trim();
      if (!clean) return '';
      if (/^area[-_\s]/i.test(clean)) return 'Area sin nombre';
    ...
    function renderEval360Rows(rows, options = {}) {
      ...
      return list.slice(0, Number(options.limit || 8)).map((row) => {
        ...
        const hasHumanItemLabel = itemLabel && itemLabel !== String(row.item_id || '').trim();
        const label = String(row.item_id
          ? ((hasHumanItemLabel ? itemLabel : '') || row.habilidad_nombre || 'Habilidad sin nombre')
          : (row.area_nombre || titleLabel || formatEval360AreaLabel(row.area_id) || 'Sin nombre')
        ).trim();
        ...
        return [
          '<div class="eval360-row">',
            '<div class="eval360-row-head">',
              ...
              '<span class="pill">' + escapeHtml(formatEval360Score(score)) + '/4</span>',
            '</div>',
    ...
    async function loadEval360AdminModule(options = {}) {
      ...
      try {
        ...
        ui.loading = false;
        renderAdminEval360Module();
        if (ui.selectedCicloId && options.loadResultados !== false) {
          setTimeout(() => {
        ...
        ui.loaded = true;
        ui.loading = false;
        ui.error = err && err.code === 'FEATURE_DISABLED' ? 'Eval360 esta apagado en este backend.' : formatApiError(err);
        renderAdminEval360Module();
      }
    }

    async function loadEval360AdminResultados(button) {
      const ui = getEval360Ui().admin;
    ...
    function renderAdminEval360Module() {
      ...
      if (ui.loading && !ui.loaded) {
        panel.innerHTML = '<article class="admin-placeholder"><h3>Cargando Evaluacion 360</h3><p>Consultando solo el modulo Eval360.</p></article>';
        return;
      }
      if (ui.error) {
        panel.innerHTML = [
          '<article class="admin-placeholder">',
            '<h3>Evaluacion 360</h3>',
            '<p>' + escapeHtml(ui.error) + '</p>',
            '<div class="actions compact"><button class="btn-secondary" type="button" onclick="refreshEval360AdminModule(this)">Reintentar</button></div>',
          '</article>'
        ].join('');
        return;
      }
        ...
        panel.innerHTML = [
          '<article class="admin-toolbar eval360-module">',
            '<div class="eval360-head">',
              '<div><h3>Evaluacion 360</h3><p class="subtle">Este modulo se carga solo cuando lo abres.</p></div>',
              '<button class="btn-primary" type="button" onclick="refreshEval360AdminModule(this)">Cargar Eval360</button>',
            '</div>',
          '</article>'
        ].join('');
        return;
      }
      if (!ui.feature || ui.feature.enabled !== true) {
        panel.innerHTML = '<article class="admin-toolbar eval360-module"><div class="eval360-disabled">Eval360 esta apagado. La app actual sigue operando sin cargar datos de este modulo.</div></article>';
        return;
      }
    ...
    function renderEval360InvitacionesTable(rows) {
      const list = Array.isArray(rows) ? rows : [];
      if (!list.length) return '<div class="eval360-empty">Todavia no hay invitaciones para mostrar.</div>';
      return list.map((row) => [
        '<div class="eval360-row">',
          '<div class="eval360-row-head"><strong>' + escapeHtml(row.alumno_id || '-') + '</strong><span class="pill">' + escapeHtml(row.estatus || '-') + '</span></div>',
    ...
    function refreshEval360AdminModule(button) {
      return handleAction('getEval360AdminData', () => loadEval360AdminModule(), {
        button,
        key: 'eval360-admin-refresh'
      });
    }
    ...
    async function loadEval360FacilitadorModule() {
      const ui = getEval360Ui().facilitador;
      ui.loading = true;
      ui.error = '';
      renderEval360FacilitadorPanel();
    ...
    function refreshEval360FacilitadorModule(button) {
      return handleAction('getEval360InvitacionesFacilitador', () => loadEval360FacilitadorModule(), {
        button,
        key: 'eval360-facilitador-refresh'
      });
    }

    async function loadEval360FacilitadorPulso(button) {
      const ui = getEval360Ui().facilitador;
      if (!ui.selectedCicloId) return;
      await handleAction('getEval360PulsoPedagogico', async () => {
        const data = await api('getEval360PulsoPedagogico', {
          ciclo_id: ui.selectedCicloId,
          momento: ui.selectedMomento
        });
        ui.pulso = data || null;
        renderEval360FacilitadorPanel();
      }, { button, key: buildActionKey('getEval360PulsoPedagogico', [ui.selectedCicloId, ui.selectedMomento]) });
    }

    function setEval360FacilitadorCiclo(value) {
      const ui = getEval360Ui().facilitador;
      ui.selectedCicloId = String(value || '').trim();
      ui.pulso = null;
    ...
    function setEval360FacilitadorMomento(value) {
      const ui = getEval360Ui().facilitador;
      ui.selectedMomento = normalizeEval360MomentFrontend(value);
      ui.pulso = null;
      clearEval360FacilitadorFormState();
    ...
    function hydrateEval360FacilitadorForm(data) {
      ...
      ui.formTouched = {};
      ui.formComments = {};
      ui.formGeneralComment = String((data && data.comentario_general) || '');
      ...
      (Array.isArray(data && data.items) ? data.items : []).forEach((item) => {
    ...
    async function loadEval360FacilitadorFormulario(invitacionId, button) {
      const ui = getEval360Ui().facilitador;
      const cleanId = String(invitacionId || '').trim();
      if (!cleanId) return;
      ui.formLoading = true;
      ui.formError = '';
      renderEval360FacilitadorPanel();
      await handleAction('getEval360FormularioFacilitador', async () => {
        const data = await api('getEval360FormularioFacilitador', { invitacion_id: cleanId });
        hydrateEval360FacilitadorForm(data);
        ui.formLoading = false;
        renderEval360FacilitadorPanel();
      }, {
        button,
        key: 'eval360-fac-form-' + cleanId,
        busyText: 'Cargando',
        onError: (err) => {
          ui.formLoading = false;
          ui.formError = formatApiError(err);
          renderEval360FacilitadorPanel();
          return false;
        }
      });
    ...
    function setEval360FacilitadorFormComment(itemId, value) {
      const ui = getEval360Ui().facilitador;
      ui.formComments[String(itemId || '').trim()] = String(value || '');
    }

    function setEval360FacilitadorFormGeneralComment(value) {
      const ui = getEval360Ui().facilitador;
      ui.formGeneralComment = String(value || '');
    }

    function isEval360FacilitadorFormItemAnswered(itemId) {
      const ui = getEval360Ui().facilitador;
      const cleanId = String(itemId || '').trim();
      if (!cleanId) return false;
      if (ui.formTouched && ui.formTouched[cleanId]) return true;
      const value = ui.formValues ? ui.formValues[cleanId] : '';
      return value !== '' && value !== null && value !== undefined;
    }

    function getEval360FacilitadorFormProgress() {
      const ui = getEval360Ui().facilitador;
    ...
    function buildEval360FacilitadorItemsPayload() {
      ...
      return items.map((item) => {
        ...
        return {
          item_id: itemId,
          area_id: String(item.area_id || '').trim(),
          valor: Object.prototype.hasOwnProperty.call(ui.formValues, itemId) ? ui.formValues[itemId] : '',
          label_snapshot: String(item.texto || '').trim(),
          comentario: String(ui.formComments[itemId] || '').trim()
        };
      });
    }

    async function saveEval360FacilitadorForm(button, enviar) {
      const ui = getEval360Ui().facilitador;
      const form = ui.form || {};
      const invitacion = form.invitacion || {};
      const invitacionId = String(invitacion.invitacion_id || '').trim();
      if (!invitacionId) return;
      if (enviar) {
        ui.formSubmitAttempted = true;
      ...
      await handleAction('guardarEval360RespuestaFacilitador', async () => {
        const data = await api('guardarEval360RespuestaFacilitador', {
          invitacion_id: invitacionId,
          respuesta_id: String(form.respuesta_id || '').trim(),
          enviar: enviar === true,
          comentario_general: ui.formGeneralComment,
          items: buildEval360FacilitadorItemsPayload()
        });
        ui.formSavedAt = new Date().toISOString();
        form.respuesta_id = String(data.respuesta_id || form.respuesta_id || '').trim();
        form.respuesta_estatus = String(data.estatus || '').trim();
        if (enviar) {
          invitacion.estatus = 'enviada';
          const inv = (ui.invitaciones || []).find((row) => String(row.invitacion_id || '').trim() === invitacionId);
          if (inv) inv.estatus = 'enviada';
        }
        renderEval360FacilitadorPanel();
        setBanner(enviar ? 'Evaluacion enviada.' : 'Borrador guardado.', 'success');
      }, { button, key: 'eval360-fac-save-' + invitacionId + '-' + (enviar ? 'send' : 'draft'), busyText: enviar ? 'Enviando' : 'Guardando' });
    }
    ...
    async function loadEval360FacilitadorSugerencias(areaId, skillId, button) {
      ...
      await handleAction('getEval360Sugerencias', async () => {
        ...
        ui.sugerencias = Array.isArray(data.plantillas) ? data.plantillas : [];
        renderEval360FacilitadorPanel();
    ...
    function renderEval360FacilitadorInvitaciones() {
      ...
      const rows = (Array.isArray(ui.invitaciones) ? ui.invitaciones : []).filter((row) => {
        if (ui.selectedCicloId && String(row.ciclo_id || '').trim() !== ui.selectedCicloId) return false;
        if (ui.selectedMomento && String(row.momento || '').trim() !== ui.selectedMomento) return false;
        return true;
      });
      if (!rows.length) return '<div class="eval360-empty">No tienes observaciones pendientes para esta seleccion.</div>';
    ...
    function renderEval360FacilitadorForm() {
      const ui = getEval360Ui().facilitador;
      if (ui.formLoading) return '<section class="eval360-panel"><div class="eval360-empty">Cargando observacion...</div></section>';
      if (ui.formError) {
        return '<section class="eval360-panel"><div class="eval360-empty">' + escapeHtml(ui.formError) + '</div></section>';
      }
      const form = ui.form || null;
      if (!form) return '';
      const inv = form.invitacion || {};
      ...
      return [
        '<section class="eval360-panel eval360-fac-form-panel" id="eval360FacilitadorFormPanel">',
          ...
          '<div class="actions compact">',
            '<button class="btn-secondary" type="button" onclick="saveEval360FacilitadorForm(this, false)"' + (sent ? ' disabled' : '') + '>Guardar borrador</button>',
            '<button class="btn-primary" type="button" onclick="saveEval360FacilitadorForm(this, true)"' + (sent || !items.length ? ' disabled' : '') + '>Enviar observacion</button>',
          '</div>',
        '</section>'
      ].join('');
    }

    function renderEval360FacilitadorFormItem(item, locked) {
      const ui = getEval360Ui().facilitador;
      const itemId = String(item.item_id || '').trim();
    ...
    function renderEval360FacilitadorPanel() {
      const panel = $('eval360FacilitadorPanel');
      if (!panel) return;
      const role = getCurrentRole();
      const ui = getEval360Ui().facilitador;
      panel.hidden = !(role === 'facilitador' && ui.open);
      ...
      if (panel.hidden) return;
      if (ui.loading && !ui.loaded) {
        panel.innerHTML = '<div class="eval360-empty">Cargando pulso pedagogico...</div>';
        return;
      }
      if (ui.error) {
        panel.innerHTML = '<div class="eval360-head"><div><h2>Evaluacion 360</h2><p class="subtle">' + escapeHtml(ui.error) + '</p></div><button class="btn-secondary" type="button" onclick="refreshEval360FacilitadorModule(this)">Reintentar</button></div>';
        return;
      }
      if (!ui.loaded) {
        panel.innerHTML = '<div class="eval360-head"><div><h2>Evaluacion 360</h2><p class="subtle">Carga el pulso solo cuando lo necesitas para planear.</p></div><button class="btn-primary" type="button" onclick="refreshEval360FacilitadorModule(this)">Cargar pulso</button></div>';
        return;
      }
      if (!ui.feature || ui.feature.enabled !== true) {
        panel.innerHTML = '<div class="eval360-disabled">Eval360 esta apagado. Planeaciones sigue igual.</div>';
        return;
      }
      const ciclos = getUniqueEval360CiclosFromInvitaciones(ui.invitaciones);
      const pulso = ui.pulso || {};
    ...
    async function startEval360PublicRoute() {
      const token = getEval360PublicTokenFromUrl();
      const ui = getEval360Ui().publicForm;
      ui.token = token;
      document.body.classList.add('eval360-public-mode');
      renderEval360PublicForm();
      await loadEval360PublicForm();
    }

    async function loadEval360PublicForm() {
      const ui = getEval360Ui().publicForm;
      ui.loading = true;
      ui.error = '';
      renderEval360PublicForm();
      try {
        const form = await api('getEval360FormularioToken', { token: ui.token });
        ui.form = form || null;
        ui.loaded = true;
        ui.loading = false;
    ...
    function buildEval360PublicItemsPayload() {
      const ui = getEval360Ui().publicForm;
      const items = ui.form && Array.isArray(ui.form.items) ? ui.form.items : [];
      return items.map((item) => {
        const itemId = String(item.item_id || '').trim();
        return {
          item_id: itemId,
          area_id: String(item.area_id || '').trim(),
          valor: Object.prototype.hasOwnProperty.call(ui.values, itemId) ? ui.values[itemId] : '',
          label_snapshot: String(item.texto || '').trim(),
    ...
    async function saveEval360PublicProgress(button) {
      const ui = getEval360Ui().publicForm;
      await handleAction('guardarEval360AvanceToken', async () => {
    ...
    async function submitEval360PublicForm(button) {
      ...
      await handleAction('enviarEval360RespuestaToken', async () => {
        if (!ui.respuestaId) {
          ...
          ui.respuestaId = String(draft.respuesta_id || '').trim();
        }
        ...
        renderEval360PublicForm();
      }, { button, key: 'eval360-public-submit' });
    }

    function renderEval360PublicForm() {
      const mount = $('eval360PublicMount');
      if (!mount) return;
      const ui = getEval360Ui().publicForm;
      if (ui.loading && !ui.loaded) {
        mount.innerHTML = '<article class="card eval360-public-shell"><h1>Evaluacion 360</h1><p class="subtle">Cargando formulario...</p></article>';
        return;
      }
      if (ui.error) {
        mount.innerHTML = [
    ...
    function renderEval360PublicItem(item) {
      const ui = getEval360Ui().publicForm;
      const itemId = String(item.item_id || '').trim();
    ...
    const windowActionGroups = {
      core: {
        ...
        toggleEval360FacilitadorPanel,
        refreshEval360FacilitadorModule,
        ...
        setEval360FacilitadorFormGeneralComment,
        saveEval360FacilitadorForm,
    ...
    async function boot() {
      ...
      if (isEval360PublicRoute()) {
        await startEval360PublicRoute();
        return;
      }
      loadSession();
