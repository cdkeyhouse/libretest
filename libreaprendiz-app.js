    ...
    const FIXED_BACKEND_URL = 'https://script.google.com/macros/s/AKfycbzF61zDQkF15kTFBQykHx-J2XAY1TFcqI_Gze_UtVZhD80DTlbkMEYTBIdQ-_HIqlSf/exec';
    ...
    function persistCurrentBootSnapshot(kind) {
      ...
      if (role === 'facilitador' && !canUseAdminShell()) {
        ...
        payload.planeacionesMeta = {
          ...
          offset: Number(state.ui && state.ui.planeacionesOffset || 0)
        };
    ...
    function restoreBootSnapshotForSession(sessionLike = state.session) {
      ...
      if (Array.isArray(snapshot.planeaciones)) {
        state.planeaciones = snapshot.planeaciones;
      }
      ...
      host.innerHTML = [
        '<div class="admin-facilitadores-summary">',
          ...
          '<div class="admin-facilitadores-meta-grid">',
            ...
            '<div class="admin-alumnos-readonly"><span>Última actividad</span><strong>' + escapeHtml(getFacilitadorUpdatedLabel(facilitador)) + '</strong></div>',
      ...
      host.innerHTML = [
        '<div class="admin-facilitadores-summary">',
          ...
          '<div class="admin-facilitadores-meta-grid">',
            ...
            '<div class="admin-alumnos-readonly"><span>Última actualización</span><strong>' + escapeHtml(taller.fecha_actualizacion ? formatFechaHumana(taller.fecha_actualizacion) : 'Sin dato') + '</strong></div>',
        ...
        '<div class="admin-alumnos-table">',
          ...
          rows.map((row) => {
            ...
            const actions = [
              ...
              '<button class="btn-ghost" type="button" onclick="event.stopPropagation(); moveMateria(this, \'' + escapeJsAttrValue(row.materia_id) + '\', \'up\')">↑</button>',
              '<button class="btn-ghost" type="button" onclick="event.stopPropagation(); moveMateria(this, \'' + escapeJsAttrValue(row.materia_id) + '\', \'down\')">↓</button>',
        ...
        '<div class="admin-materias-variants">',
          subRows.map((row) => {
            const actions = [
              ...
              '<button class="btn-ghost" type="button" onclick="moveSubmateria(this, \'' + escapeJsAttrValue(materia.materia_id) + '\', \'' + escapeJsAttrValue(row.submateria_id) + '\', \'up\')">↑</button>',
              '<button class="btn-ghost" type="button" onclick="moveSubmateria(this, \'' + escapeJsAttrValue(materia.materia_id) + '\', \'' + escapeJsAttrValue(row.submateria_id) + '\', \'down\')">↓</button>',
              (row.estatus === 'activa'
    ...
    function renderPlaneacionesList() {
      ...
      if (planeacionesLoading && !planeacionesLoaded && !entriesToRender.length) {
        host.innerHTML = buildPlaneacionesListSkeleton();
        return;
      }
      ...
      host.innerHTML = (focusedEntry ? focusBar : desktopHeader) + entriesToRender.map((entry) => {
        ...
        if (!isOpen) {
          const quickActivateButton = plan.estado === 'borrador'
        ...
        return (
          '<article id="plan-card-' + escapeHtml(plan.planeacion_id) + '" class="plan-card">' +
            ...
            '<div class="stack">' +
              ...
              obsGeneralesHtml +
              '<div class="actions compact">' +
                '<textarea id="obs-general-' + escapeHtml(plan.planeacion_id) + '" placeholder="Agregar observación general para administración"' + (allowGeneralObs ? '' : ' disabled') + '></textarea>' +
    ...
    function formatAlertDescription(alerta) {
      ...
      if (tipo === 'material_pendiente' && descripcion.indexOf('Material no confirmado en carpetas') === 0) {
        const nombre = descripcion.split('—').slice(1).join('—').trim();
        return nombre
    ...
    function renderMaintenanceSheetRows() {
      ...
      if (!rows.length) {
        return '<div class="admin-alumnos-empty"><div><strong>Sin datos cargados.</strong><div class="subtle">Usa “Actualizar vista previa” para revisar qué se limpiaría.</div></div></div>';
      }
      ...
      return [
        '<div class="admin-config-module">',
          '<article class="admin-placeholder">',
            ...
            '</div>',
            lastReset
              ? ('<div class="admin-note">Último reset registrado: ' + escapeHtml(lastReset.at || '-') + ' por ' + escapeHtml(lastReset.by || '-') + (lastReset.role ? ' (' + escapeHtml(lastReset.role) + ')' : '') + '.</div>')
    ...
    async function savePlanEditor(targetStatusOverride) {
      ...
      if (!$('planMateria').value) throw new Error('Selecciona una materia.');
      const selectedSubmateriaId = $('planSubmateria') ? $('planSubmateria').value : '';
      if (materiaRequiresPlanSubmateria($('planMateria').value) && !selectedSubmateriaId) {
      ...
      if (editorMode === 'edit') {
        responseData = await api('guardarPlaneacionCompleta', {
          ...
          frase_semana: $('planFrase').value.trim(),
          alumnos_ids: alumnosIds,
          actividades,
        ...
        const targetStatus = String(targetStatusOverride || '').trim() === 'activa' ? 'activa' : 'borrador';
    ...
    async function planAction(button, planId, action) {
      await handleAction(action, async () => {
        const shouldCloseOpenCard = action === 'activarPlaneacion' && state.openPlanId === planId;
        if (shouldCloseOpenCard) {
          state.openPlanId = '';
          state.openPlanDraft = null;
        }
        const response = await api(action, { planeacion_id: planId, request_id: uid('PLAN') });
    ...
    function applySavedPlaneacionDetail(planId, updatedPlan) {
      ...
      upsertPlaneacionRow(updatedPlan);
      state.openPlanId = planId;
      state.openPlanDraft = buildOpenPlanDraft(updatedPlan);
    }
    ...
    async function savePlanChanges(button, planId, entryKey) {
      ...
      await handleAction('guardarCambiosPlaneacion', async () => {
        ...
        if (canPatchSimplePlanLocally && !shouldRefetchPlaneacionesAfterPlanSave(plan, updatedPlan)) {
          ...
          state.openPlanDraft = null;
          await refreshPlaneacionesSurface({ includeAlertas: shouldSavePlan || shouldSaveShared });
        }
    ...
    function buildClosePlanPayload(planId, fallbackPlan) {
      ...
      for (const item of activityRows) {
        ...
        if (!['si', 'no'].includes(realizada)) {
          const error = new Error('Debes confirmar si se realizó la actividad ' + String(item.orden || '?') + '.');
          error.focusTargetId = realizadaFieldId;
          throw error;
        }
        if (realizada === 'no' && !comentario) {
          const error = new Error('La actividad ' + String(item.orden || '?') + ' necesita comentario porque no se realizó.');
